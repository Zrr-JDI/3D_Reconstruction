...
int main()
{
    ...
    vector<cv::Mat> Rs;                               // 每张相机的旋转矩阵 (3x3)
    vector<cv::Mat> ts;                               // 每张相机的平移向量 (3x1)
    vector<cv::Mat> Ks;                               // 每张相机的内参矩阵 (3x3)
    vector<cv::Point3d> points3D;                     // 全局三维点坐标（世界坐标系）
    vector<vector<cv::Point2d>> projections2D_all; // 每个相机上所有三维点的二维投影坐标
    vector<vector<int>> viewIndices;             // 每个3D点在哪些相机中被观测到（相机索引）
    vector<cv::Mat> images;                            // 对应的原始图像（用于提取颜色）
